...
%Devising algorithms that maintain consistency in the
%presence of failures and in the absence of synchrony assumptions is a
%major challenge in distributed computing. 
%
...
\includegraphics[scale=0.45]{Figures/scs_other.pdf}
   \subcaption{\footnotesize{$\Chr^1(\s)$, equivalent to the output complex of the IS task.}}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{.49\linewidth}
    \begin{center}
\includegraphics[scale=0.5]{Figures/1-res.pdf}
     \subcaption{\footnotesize{$T_1$, the task corresponding to $1$-resilience.}}
    \end{center}
  \end{minipage}
  \caption{\small{Examples of subsets of iterated standard chromatic subdivisions of
  $\s$ in dimension $2$.}}
  \label{Fig:scs}
\end{figure}
...
Thus, the topological characterization of Herlihy
and Shavit~\cite{HS99} can be interpreted as:
the set of wait-free solvable task is precisely the set of tasks
solvable in the IIS model. 
The ability of (iteratively) solving the IS task allows us
...
metric~\cite{AS85}, the model is \emph{non-compact}.
Indeed, all finite prefixes of an infinite run can be in the model,
...
vector with a set of possible output vectors. An input~$\bot$ 
denotes a \emph{non-participating} process and an output 
...
prefix $R$ of the run where: (1)~decided values form a 
vector $O\in\O$ such that $(I,O)\in\Delta$, and (2)~all 
correct processes decide. 

\remove{
\myparagraph{$k$-set-consensus and $k$-concurrency models.}
%
A $k$-set-consensus object 
can be accessed with a \emph{propose} operation
that takes a \emph{proposed} value as an argument and returns a
\emph{decided} value as a response, so that, in each run,  decided
values constitute a subset of proposed values of size at most $k$.   
For $k\in\{1,\ldots,n-1\}$, in the \emph{$k$-set-consensus model}, processes communicate via the
AS memory and $k$-ste-consensus objects. 

Let $R$ be a finite run of the AS model.
A process $p_i$ is called \emph{active at the end of $R$} 
if $p_i$ participates in $R$ but has not terminated by the end of $R$. 
Let $\textit{active}(R)$ denote the set of all processes that 
are active at the end of $R$.

A run $R$ of the AS model is \emph{$k$-concurrent} ($k=1,\ldots,n$) if at most 
$k$ processes are \emph{concurrently active} in $R$, i.e., 
$\max\{|\textit{active}(R')|;\; R' \textrm{ prefix of } R\}\leq k$. 
The \emph{$k$-concurrency} model is the set of $k$-concurrent 
runs of the AS model.

It is known that the $k$-concurrency model is \emph{equivalent} 
to the $k$-set-consensus model~\cite{GG09}\footnote{In fact, 
this paper contains a self-contained proof of this equivalence 
result.}: any task that can be solved $k$-concurrently can also 
be solved in the $k$-set-consensus model, and vice versa.
}
...
%
We employ a the \emph{BGG-simulation} simulation
technique~\cite{KR17} that
assembles algorithmic ideas presented in~\cite{BG93a,Gaf09-EBG,GG09,GG11-univ}. 
%Let us now recall the main simulation
%tool that is used in this paper, that will call the BGG-simulation
%according to the name of the three main investigators of this
%technique, namely Elizabeth Borowski, Eli Gafni and Rachid
%Guerraoui.
...
\myparagraph{Defining models via agreement functions.}
%
\begin{definition}{[Agreement function]}
The \emph{agreement function} of an (adversarial) model $\A$ is 
...
$\alpha(P)$-set consensus can be solved, 
...
\begin{definition}{[Solving a task $\alpha(P)$-active-resiliently]}
Let $\alpha: 2^{\Pi} \to \{0,\ldots,n\}$ be the agreement function of a
model $\A$.
Let $P$ be any subset of $\Pi$. 
An algorithm \emph{solves a task $T=(\I,\O,\Delta)$ in $\alpha(P)$-active-resiliently} if,
assuming that $P$ is the participating set, it ensures that (1) in every run in which
processes start with an input vector $I\in\I$, 
all decided values form a vector $O\in\O$ such that $(I,O)\in\Delta$,
 and (2) if the run is $(\alpha(P)-1)$-active-resilient, i.e., at most
 $\alpha(P)-1$ participating processes are faulty. 
\end{definition}

We are going to use the following result concerning 
}

%======================
%\section{Defining models through agreement functions}

%\trnote{Previous version way too long for here, a section is probably too much also. Last, it may be convenient to make noticed that agreement function extended to the number of ``active'' processes, i.e., the number of processes participating to a particular agreement operation trivially capture collection of set-consensus objects...}
...
%\myparagraph{Models.}
%
Generally, a \emph{model} is a set of runs. 
In the AS case, we assume that a model allows every process to
participate, i.e., for each $P\in2^{\Pi}$, the model includes a run in
which $P$ is the participating set. 

Consider a function $\alpha: 2^{\Pi} \to \{0,\ldots,n\}$. 
We say that $\alpha$ is an agreement function of if each set of participating processes
$P\in 2^{\Pi}$ can solve $\alpha(P)$-set agreement but cannot solve
$(\alpha(P)-1)$-set agreement in this model.  
By convention, $\alpha(P)=0$ implies that if $P$ is the participating
set, then no process is obliged to decide.

We say that $\alpha$ is \emph{monotonic} if for all $P$ and $P'$ in $2^{\Pi}$,
$P\subseteq P'$ implies $\alpha(P)\leq\alpha(P')$. 



\remove{
\myparagraph{Active resilience.}  
If in a given run, at most $k\in\Nat_n$ participating
processes are faulty and at least one participating process is
correct, we say that the run is \emph{$k$-active-resilient}.   
The model of  \emph{$k$-active resilience} consists of all
$k$-active-resilient runs. 

It has been shown in~\cite{GG11-univ} that the model of $k$-active
resilience is able to solve exactly the same set of tasks as the the
model in which, in addition to atomic snapshots, processes are allowed
to access $(k+1)$-set agreement objects, or the model of
$k$-concurrency in which at most $k$ processes can be concurrently
\emph{active}, i.e., after they take the first step and before they
produce outputs. 

\begin{lemma}%~\cite{GG11-univ}
\label{lem:kconc}
The agreement function of the model of $k$-active-resilience is
$\alpha_{k,\textit{conc}}(P)= \min(|P|,k+1)$.
\end{lemma}    
}
